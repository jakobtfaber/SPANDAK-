...
def _cdd_auto(cdd=True):
	...
	for B in np.arange(len(start_times)):
		...
		+ '/datax/scratch/jfaber/SPANDAK_extension/pipeline_playground/R3_GBT_1/GBT_GMRT_paper/bursts_bpc/' + par_fil_paths[B] + ' -s -a psrfits -e fits ' \
		+ '/datax/scratch/jfaber/SPANDAK_extension/pipeline_playground/R3_GBT_1/GBT_GMRT_paper/bursts_bpc/' \
		+ 'spliced_' \
	...
	if cdd == True:
		...
		for cdd in cdd_run_commands:
			...
			time_dir = '/datax/scratch/jfaber/SPANDAK_extension/pipeline_playground/R3_GBT_1/GBT_GMRT_paper/bursts_bpc/' + str(cdd.split('/')[9])
			fits_dir = '/datax/scratch/jfaber/SPANDAK_extension/pipeline_playground/R3_GBT_1/GBT_GMRT_paper/bursts_bpc/' + str(cdd.split('/')[9]) + '/fits'
			#print('Time', time_dir)
			#print('Fits', fits_dir)
