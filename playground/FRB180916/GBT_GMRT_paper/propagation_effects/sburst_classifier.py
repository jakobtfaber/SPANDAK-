...
def subband(intensity_array, sub_factor):
    ...
    npy = np.load(str(intensity_array))
    print('Numpy Array Loaded')
    sub_factor = np.int(sub_factor)

    # Sub-band frequency channels
    arr = np.nanmean(
        npy.reshape(-1, sub_factor, npy.shape[1]), axis=1
    )

    freq = np.linspace(600, 800, arr.shape[1])
...
def find_subbursts(snapshot_data, eventid, sub_factor=32, plot_dyn=False,
    ...
    arr, freq = subband(intensity_array, sub_factor)
...
if __name__ == "__main__":
    ...
    intensity_array = sys.argv[1]
    eventid = sys.argv[2]
    centroid_coord_x_array, centroid_coord_x_units, \
    centroid_coord_y_array, centroid_coord_y_units, \
    centroid_prominence, centroid_widths, component_number,\
    centroid_drift_rate, sidelobe_detection, centroid_bandwidths = find_subbursts(intensity_array, eventid)
