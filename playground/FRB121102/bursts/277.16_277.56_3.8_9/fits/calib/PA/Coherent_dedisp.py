...
#   If low and high are given, use them. If not then define a window
	...
	plt.scatter(np.arange(len(PAerr)),PAerr)
 	plt.plot(np.arange(1, len(y)+1), y,linewidth=0.5,color='black')
	plt.plot(np.arange(1, len(Iprof3)+1), Iprof3,linewidth=0.5,color='blue')	
	plt.show()
	sys.exit(0)
	#Diag end
	'''
...
if __name__ == "__main__":
	...
	cmd = 	"time dspsr -U 1024 -T %.1f -F 6656:D -K -d 4 -b 2048  -E /home/vgajjar/FRB180301.par -s -a psrfits -e fits %s" % (totime,infile)
	simrun(gonogo,cmd)
